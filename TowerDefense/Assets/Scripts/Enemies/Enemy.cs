...
{
    [field: SerializeField] private Transform _model;
    ...
    private void PrepareTurnRight()
    {
        ...
        _progressFactor = _speed / ((float)Math.PI * 0.5f * (0.5f - _pathOffset));
    }
    ...
    {
        _directionAngleTo = _directionAngleFrom + (_pathOffset < 0f ? 180 : -180);
        _model.localPosition = new Vector3(_pathOffset, 0f);
        transform.localPosition = _positionFrom;
        _progressFactor = _speed / ((float)Mathf.PI * Mathf.Max(MathF.Abs(_pathOffset), 0.2f));
    }
